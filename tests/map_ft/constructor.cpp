...
void    constructor_range_map()
{
  ...
  ft::map<std::string, int>::iterator it_2 = map_arr.begin();
...
void    operator_assignation_map()
{
    ...
    map_1["m"] = 54644587;
    map_2 = map_1;
    for (ft::map<std::string, int>::iterator	it = map_2.begin(); it != map_2.end(); it++)
		  std::cout << "it->first : " << it->first << " it->second : " << it->second << std::endl;
    ...
    for (ft::map<std::string, int>::iterator	it = map_1.begin(); it != map_1.end(); it++)
		  std::cout << "it->first : " << it->first << " it->second : " << it->second << std::endl;
}
...
void  assignation_speed()
{
  ...
  map_2 = map;
  ft::map<double, double>::iterator  it = map_2.end();
  static_cast<void>(it);
}
...
void  constructor_range_speed()
{
  ...
  map.insert(map.find(50000), ft::make_pair(10000000, 123));
  map.insert(map.find(98000), ft::make_pair(20000000, 123));
  ft::map<double, double> map_2(map.begin(), map.end());
  ft::map<double, double>::iterator  it = map_2.end();
  static_cast<void>(it);
}
...
void  constructor_copy_speed()
{
  ...
  for (double i = 0; i < 40000; i++)
    map[i] = i;
  ft::map<double, double> map_2(map);
}
